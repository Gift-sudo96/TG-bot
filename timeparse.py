...
# Прийменник мусить бути ОКРЕМИМ словом: без цієї перевірки з "до 10 фото"
# вихоплювалось "о 10" і ставало десятою годиною.
BOUND = r"(?<![\wа-яіїєґ])"
# Для голої години "до" не беремо: "до 10 фото" трапляється частіше,
# ніж "до 10" у значенні часу. Там, де є двокрапка, воно однозначне.
PREP_H = r"(?:о|об|у|в|на)"

RE_HHMM = re.compile(
    r"(?:" + BOUND + r"(" + PREP + r")\s+)?(?<![\d])(\d{1,2})\s*[:.\-]\s*"
    r"(\d{2})(?![\d])")
...
RE_H_STRICT = re.compile(
    BOUND + r"(?:(о|об)\s+)(?<![\d])(\d{1,2})(?![\d])(\s*годин\w*|\s*год)?"
    r"(?![\wа-яіїєґ])", re.I)
RE_H_LOOSE = re.compile(
    BOUND + r"(?:(" + PREP_H + r")\s+)(?<![\d])(\d{1,2})(?![\d])"
    r"(\s*годин\w*|\s*год)?(?![\wа-яіїєґ])", re.I)

# "зараз" окремим рядком — доставити негайно. Саме окремим: у звичайному
# реченні ("скільки зараз лишилось") це слово часу не позначає.
RE_NOW = re.compile(r"^[\s\-–—]*(зараз|негайно|терміново|сейчас)[!.…\s]*$",
                    re.I | re.M)
...
def parse(text, now):
    ...
    if m:
        spans.add(m)
        # хвилина запасу, щоб підтвердження не писало "цей час уже минув"
